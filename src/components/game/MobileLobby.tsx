...
import { Gem, Coins, Users, Store, Sword, Star, Grid2X2, Gift } from 'lucide-react';
...
interface MobileLobbyProps {
  playerName: string;
  level: number;
  exp: number;         // progreso actual de exp
  expMax: number;      // exp máxima para subir de nivel
  coins: number;
  gems: number;
  tickets?: number;    // si usás algún recurso extra
...
export default function MobileLobby({
  ...
  return (
    <div className="relative h-screen w-full max-w-md mx-auto bg-[radial-gradient(circle_at_1px_1px,#0b1120_1px,transparent_0)] bg-[length:22px_22px] overflow-hidden">
      {/* CONTENEDOR PRINCIPAL SCROLLABLE (dejamos espacio abajo para la nav) */}
      <div className="flex flex-col h-full pb-20">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              <div className="flex flex-col items-center">
                <div className="flex items-center justify-center rounded-full bg-sky-500 border-[3px] border-black w-10 h-10 shadow-[0_3px_0_#020617]">
        ...
        <main className="flex-1 overflow-y-auto px-3 pb-2 space-y-3">
          {/* PANEL PRINCIPAL (modo de juego / mapa) */}
          <motion.section
            layout
          ...
          >
            <div className="flex items-center justify-between mb-1">
              ...
              {/* Mini indicador de dificultad / rango */}
              <div className="flex items-center gap-1">
            ...
            {/* Mapa / preview del tablero - sustituí esta imagen por tu ilustración */}
            <div className="mt-1 relative w-full aspect-[16/11] rounded-2xl border-[3px] border-black bg-sky-500/30 overflow-hidden shadow-[0_6px_0_#020617]">
              {/* Placeholder de mapa */}
            ...
            {/* Progreso de copa / misiones, tipo barra horizont */}
            <div className="mt-3 flex flex-col gap-2">
              <div className="flex items-center justify-between text-xs text-slate-100">
                <span className="comic-section-title !text-[9px]">Progreso de temporada</span>
              ...
              <div className="flex justify-between mt-1">
                {Array.from({ length: 4 }).map((_, i) => (
                  ...
                  >
                    <ChestIcon rarity={i === 3 ? 'epic' : i === 2 ? 'rare' : 'common'} />
                  </div>
                ))}
              </div>
            </div>

            {/* BOTÓN DE BATALLA PRINCIPAL */}
          ...
          {/* SECCIÓN RESUMEN ABAJO DEL PANEL (según tab activo) */}
          <section className="space-y-2">
            {activeTab === 'battle' && (
              <LobbyInfoCard
                title="Misión diaria"
                subtitle="Ganá 3 partidas para conseguir un cofre especial."
            ...
            {activeTab === 'collection' && (
              <LobbyInfoCard
                ...
                subtitle="Mejorá tus personajes para explotar el tablero."
                icon={<Grid2X2 className="w-5 h-5 text-sky-300" />}
              />
            )}
            {activeTab === 'shop' && (
              <LobbyInfoCard
                title="Tienda"
                subtitle="Ofertas limitadas de cofres, gemas y cosméticos."
                icon={<Store className="w-5 h-5 text-emerald-300" />}
              />
            )}
            {activeTab === 'club' && (
              <LobbyInfoCard
                title="Club de Bombers"
                subtitle="Unite a un club para compartir estrategias y duelos amistosos."
            ...
            {activeTab === 'events' && (
              <LobbyInfoCard
                title="Eventos"
                subtitle="Modos especiales con reglas locas y recompensas épicas."
